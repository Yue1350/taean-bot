...
class MeetupView(discord.ui.View):
    ...
    async def edit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.author.id:
            await interaction.response.send_message("⚠ 주최자만 정모 내용을 수정할 수 있습니다.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send("⚠ 입력 시간이 초과되어 수정이 취소되었습니다.", ephemeral=True)
    ...
    async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id == self.author.id:
            ...
            await interaction.response.send_message("⚠ 주최자만 투표를 마감할 수 있습니다.", ephemeral=True)
...
class MeetupCog(commands.Cog):
    ...
    async def create_meetup(self, interaction: discord.Interaction, info: str):
        ...
        try:
            ...
            await interaction.followup.send("⚠️ **메시지 고정 및 관리** 권한이 없어 고정 메시지를 처리하지 못했습니다.", ephemeral=True)
